...
<div class="borderwrap">
	<form name="upload" action="{S_ALBUM_ACTION}" method="post" enctype="multipart/form-data" class="ipbform2">
		<div class="maintitle"><h3>{L_UPLOAD_PIC}</h3></div>
	...
	<fieldset>
		...
		<dl>
			...
			<dd id="upload_file_row"><input type="file" name="pic_file[]" />
				<!-- BEGIN switch_multiple_uploads -->
				<span id="addbutton" class="gen">&nbsp;<input type="button" value="{L_ADD_FILE}" onclick="AddFileField()" class="button" /></span>
				<!-- END switch_multiple_uploads -->
		...
		<dl>
			<dt>&nbsp;</dt>
			<dd><input type="button" value="{DELETE_ALL_FIELD}" onclick="RemoveFileField()" class="button" /></dd>
		</dl>
		<!-- END switch_multiple_uploads -->
		<!-- END switch_nuffload_disabled -->
	...
	<div class="formbuttonrow center">
			...
			<!-- END switch_nuffload_enabled -->
			<!-- BEGIN switch_nuffload_disabled -->
			<input type="reset" value="{L_RESET}" class="button" onclick="InitForm();" />&nbsp;
			<input type="submit" name="submit" value="{L_SUBMIT}" class="button" />
			<!-- END switch_nuffload_disabled -->
	</div>
<script type="text/javascript">
