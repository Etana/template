# Template searchresults.L_TOPIC_ON
* __Type :__ sous-variable d'affichage
* __Utilisable dans :__ [`search_results_posts`](../tpl/search_results_posts.md#readme)
* __Utilisation :__

```html
{searchresults.L_TOPIC_ON}
```

## Description[*](https://fa-tvars.appspot.com/var/searchresults.L_TOPIC_ON)
[*Ajouter une description*](https://fa-tvars.appspot.com/var/searchresults.L_TOPIC_ON)

## Utilisations dans les templates

### Version phpBB3
* __[`search_results_posts`](../tpl/search_results_posts.md#readme) :__ lignes [`16`](../src/prosilver/search_results_posts.tpl#L16)[`<->`](../src/prosilver/search_results_posts.tpl#L16-L16)[`16`](../src/prosilver/search_results_posts.tpl#L16)

