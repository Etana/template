# topics_list_box.row.header_table.pagination


Attribut de [topics_list_box.row.header_table](topics_list_box.row.header_table.md#readme).

```html
<!-- BEGIN topics_list_box -->
    <!-- BEGIN row -->
        <!-- BEGIN header_table -->
        <!-- BEGIN topics_list_box.row.header_table.pagination -->
        contenu
        <!-- END topics_list_box.row.header_table.pagination -->
        <!-- END header_table -->
    <!-- END row -->
<!-- END topics_list_box -->
```

##Description
[*ajouter*](https://fa-tvars.appspot.com/var/topics_list_box.row.header_table.pagination)

## Utilisations dans les templates

### Version phpBB2
* __[`topics_list_box`](../tpl/var/subsilver/topics_list_box.md#readme) :__ lignes [`33`](../tpl/src/subsilver/topics_list_box.tpl#L33), [`37`](../tpl/src/subsilver/topics_list_box.tpl#L37)