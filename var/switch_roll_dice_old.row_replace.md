# switch_roll_dice_old.row_replace
* __Type :__ attribut de bouclage
* __Disponible dans :__ une boucle [`<!-- BEGIN switch_roll_dice_old -->...<!-- END switch_roll_dice_old -->`](https://github.com/Etana/template/blob/master/var/switch_roll_dice_old.md#readme)
* __Utilisation :__

```html
<!-- BEGIN switch_roll_dice_old -->
    <!-- BEGIN row_replace -->
    contenu
    <!-- END row_replace -->
<!-- END switch_roll_dice_old -->
```

## Description
[*Ajouter une description*](https://fa-tvars.appspot.com/var/switch_roll_dice_old.row_replace)

## Attributs
* __[`{switch_roll_dice_old.row_replace.DICE_ID}`](switch_roll_dice_old.row_replace.DICE_ID.md#readme)__
* __[`{switch_roll_dice_old.row_replace.DICE_NAME}`](switch_roll_dice_old.row_replace.DICE_NAME.md#readme)__


## Utilisations dans les templates

### Version PunBB
* __[`posting_body`](../tpl/var/posting_body.md#readme) :__ lignes [`243`](../tpl/src/punbb/posting_body.tpl#L243)[`<->`](../tpl/src/punbb/posting_body.tpl#L243-L245)[`245`](../tpl/src/punbb/posting_body.tpl#L245), [`323`](../tpl/src/punbb/posting_body.tpl#L323)[`<->`](../tpl/src/punbb/posting_body.tpl#L323-L325)[`325`](../tpl/src/punbb/posting_body.tpl#L325)

### Version phpBB2
* __[`posting_body`](../tpl/var/posting_body.md#readme) :__ lignes [`297`](../tpl/src/subsilver/posting_body.tpl#L297)[`<->`](../tpl/src/subsilver/posting_body.tpl#L297-L299)[`299`](../tpl/src/subsilver/posting_body.tpl#L299), [`374`](../tpl/src/subsilver/posting_body.tpl#L374)[`<->`](../tpl/src/subsilver/posting_body.tpl#L374-L376)[`376`](../tpl/src/subsilver/posting_body.tpl#L376)